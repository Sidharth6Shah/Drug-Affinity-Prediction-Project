# Training script for gnn_iter5 (with cross-attention)
import torch
...
from gnn_iter5 import BindingAffinityGNN
...
def trainEpoch(model, dataloader, optimizer, criterion, device, max_grad_norm=1.0):
    model.train()
    totalLoss = 0
...
def main():
    ...
    print(f"  Weight decay:      1e-3 (0.001)")
    print(f"  Batch size:        {batchSize}")
    print(f"  Gradient clipping: 1.0")
    print(f"  Dropout:           0.3 (MLP)")
    print("="*70)
    ...
    for epoch in range(numEpochs):
        ...
        # Train
        trainLoss = trainEpoch(model, trainLoader, optimizer, criterion, device, max_grad_norm=1.0)
        ...
        if valRmse < bestValRmse:
            bestValRmse = valRmse
            patienceCounter = 0
            torch.save(model.state_dict(), 'results/best_gnn_iter5_model.pt')
    ...
    print("="*70)
    model.load_state_dict(torch.load('results/best_gnn_iter5_model.pt'))
    ...
    results = {
        'model': 'GNN_iter5',
        'architecture': 'GNN + Cross-Attention + MLP',
        'hyperparameters': {
    ...
    with open('results/gnn_iter5_results.json', 'w') as f:
